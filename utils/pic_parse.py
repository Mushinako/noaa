...
def _parse_lat(tags: dict[str, Any]) -> float:
    """"""
...
def _parse_lon(tags: dict[str, Any]) -> float:
    """"""
    lon_tag = tags.get("GPS GPSLongitude")
    lon_tag_ref = tags.get("GPS GPSLongitudeRef")
...
def _parse_datetime(tags: dict[str, Any]) -> datetime.datetime:
    """"""
    dt_tag = (
    ...
    dt_offset_tag = (
        tags.get("EXIF OffsetTimeOriginal")
        or tags.get("EXIF OffsetTime")
        or tags.get("EXIF OffsetTimeDigitized")
    )
...
def get_pic_metadata(path: Path) -> _MetaData:
    ...
    return _MetaData(
        lat=_parse_lat(tags), lon=_parse_lon(tags), datetime=_parse_datetime(tags)
    )
